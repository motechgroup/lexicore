<div>
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="font-display-lg text-3xl text-primary dark:text-white font-bold">Retainer Statement</h1>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            @if($invoice->status === 'unpaid')
                <button wire:click="markAsPaid" class="px-4 py-2.5 bg-emerald-600 hover:opacity-90 active:scale-98 text-white font-semibold text-xs rounded-xl transition-all shadow-md shadow-emerald-900/10 flex items-center gap-1.5 justify-center">
                    <span class="material-symbols-outlined text-[18px]">check_circle</span>
                    Mark as Paid
                </button>
            @else
                <button wire:click="markAsUnpaid" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700/80 text-slate-705 dark:text-slate-300 font-semibold text-xs rounded-xl transition-all flex items-center gap-1.5 justify-center">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-8 shadow-sm max-w-4xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start border-b border-slate-100 dark:border-slate-850 pb-6 mb-6 gap-6">
            ...
            <div class="text-left md:text-right space-y-1 text-xs">
                <p class="text-slate-400">Statement ID: <span class="font-semibold text-slate-850 dark:text-slate-200 font-mono">{{ $invoice->invoice_number }}</span></p>
                <p class="text-slate-400">Issue Date: <span class="font-semibold text-slate-850 dark:text-slate-200">{{ $invoice->created_at->format('M d, Y') }}</span></p>
        ...
        <div class="flex flex-col items-end text-xs space-y-2 border-b border-slate-100 dark:border-slate-850 pb-6 mb-6">
            <div class="flex justify-between w-64">
                ...
                <span class="font-bold text-slate-800 dark:text-slate-200">${{ number_format($invoice->subtotal, 2) }}</span>
            </div>
            @if($invoice->tax_amount > 0)
                <div class="flex justify-between w-64">
                    <span class="text-slate-400">Tax ({{ $invoice->tax_rate }}%):</span>
                    <span class="font-bold text-slate-800 dark:text-slate-200">${{ number_format($invoice->tax_amount, 2) }}</span>
                </div>
            @endif
            @if($invoice->discount > 0)
                <div class="flex justify-between w-64">
                    <span class="text-slate-400">Discount Applied:</span>
                    <span class="font-bold text-red-500">-${{ number_format($invoice->discount, 2) }}</span>
                </div>
            @endif
            <div class="flex justify-between w-64 pt-2 border-t border-slate-100 dark:border-slate-850 text-sm">
                ...
                <span class="font-extrabold text-primary dark:text-white text-base">${{ number_format($invoice->total, 2) }}</span>
            </div>
        </div>
